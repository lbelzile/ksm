...
double sumsignedlog(const arma::vec &x, LogicalVector sgn){
  int n = sgn.length();
  if((int) x.n_elem != n){
    stop("Invalid arguments: \"x\" and \"sgn\" must match.");
  }
...
arma::vec dWishart(const arma::cube &x, double df, const arma::mat &S, bool log = false){
  ...
  cst -= 0.25 * d * (d - 1.0) * std::log(datum::pi);
  cst -= 0.5 * df * logdetS;
  for(arma::uword i=0; i<n; i++){
   logdens(i) = cst + 0.5 * (df - d - 1) * arma::log_det_sympd(x.slice(i)) -
   	0.5 * arma::trace(arma::solve(S, x.slice(i), solve_opts::likely_sympd));
  }
  if(log){
    return logdens;
  } else{
...
double dWishart_mat(const arma::mat &x, double df, const arma::mat &S, bool log = false){
  ...
  double logdens = cst + 0.5 * (df - d - 1) * arma::log_det_sympd(x) -
      0.5 * arma::trace(arma::solve(S, x, solve_opts::likely_sympd));
if(log){
    return logdens;
  } else{
...
arma::vec dsmnorm(const arma::cube &x, double b, const arma::mat &M, bool log = true){
  ...
  double logcst;
  logcst = 0.25 * d * (d + 1.0) * std::log(2.0 * datum::pi * b) - 0.25 * d * (d - 1.0) * std::log(2);
  arma::vec logdens(n);
  for(arma::uword i = 0; i < n; i++){
   logdens(i) = - accu(arma::pow(x.slice(i) - M, 2.0)) / (2.0 * b) - logcst;
  }
  if(log){
    return logdens;
  } else{
...
double dsmnorm_mat(const arma::mat &x, double b, const arma::mat &M, bool log = true){
  ...
  double logcst;
  logcst = 0.25 * d * (d + 1.0) * std::log(2.0 * datum::pi * b) - 0.25 * d * (d - 1.0) * std::log(2);
    double logdens = - accu(arma::pow(x - M, 2.0)) / (2.0 * b) - logcst;
  if(log){
    return logdens;
  } else{
    return std::exp(logdens);
...
arma::vec mgamma(const arma::vec &x, int p, bool log = false){
 ...
 arma::vec out(x.n_elem, fill::value(0.25 * p * (p - 1) * std::log(datum::pi)));
 for(int i = 1; i <= p; i++){
...
double lmgamma(double x, arma::uword p){
  ...
  double out = 0.25 * p * (p - 1) * std::log(datum::pi);
...
Rcpp::List solvericcati(
  ...
  return Rcpp::List::create(
    ...
    Named("error") = err,
    Named("niter") = niter,
    Named("convergence") = niter < (maxiter - 1)
    );
}
...
double lcvkernWishart(const arma::cube &x, const double &b){
   ...
   logcst -= 0.25 * d * (d - 1.0) * std::log(datum::pi);
   int it;
   for(uword k = 0; k < n; k++){
     it = 0;
     loo.zeros();
...
NumericVector kdensWishart(
    ...
    const double &b,
    bool log = true){
  uword d = x.n_rows;// [[Rcpp::export]]
  uword n = x.n_slices;
  uword m = pts.n_slices;
  arma::vec logdet_x(n);
...
// VAR1
// WAR1
// LSCV for Wishart/symmetric matrix-variate log-Gaussian kernel
// LSCV - numerical integration with cubature
// LSC Monte Carlo
// rmbeta2
...
arma::vec lscvkernwishart(
 ...
 logcst2 -= 0.25 * d * (d - 1.0) * std::log(datum::pi);
...
// [[Rcpp::export(lscv_kern_smlnorm)]]
 arma::vec lscvkernsmlnorm(
   ...
   double logcst2;
   logcst2 = 0.25 * d * (d + 1.0) * std::log(2.0 * datum::pi * b) - 0.25 * d * (d - 1.0) * std::log(2);
     for(uword i = 0; i < n; i++){
     nh = 0;
     logkern.zeros();
     logetr.zeros();
...
NumericMatrix rmnorm(int n, const arma::vec &mean, const arma::mat &vcov){
   ...
   return Rcpp::as<Rcpp::NumericMatrix>(wrap(samp));
 }
